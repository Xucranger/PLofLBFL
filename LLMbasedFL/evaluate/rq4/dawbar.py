...
def draw_rawdiff_pic(path,dataset,arr):
    ...
    # Create bars
    rects1 = ax.bar(ind - width, count, width, label='Top-1', color=color1, edgecolor='black', linewidth=2)
    ...
    def autolabel(rects):
        for rect in rects:
            ...
            ax.annotate('{}'.format(height),
                        ...
                        xytext=(0, 3),  # 3 points vertical offset
                        textcoords="offset points",
                        ha='center', va='bottom', fontsize=dynamic_fontsize)  # 动态调整字体大小)
...
def analyze_BugT(experiment_index, model_name, rangeIndex,root_path,havelegend):
    ...
    # draw_rawdiff_pic(title,model_name,difficultyrawlist)
