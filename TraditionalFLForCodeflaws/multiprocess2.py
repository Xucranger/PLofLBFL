...
def FLwork(proId, versionId):
    ...
    srcPath = os.path.join(util.rootSourcePath, proId, "test_data", "defect_root", "source", "tcas.c")
    source = util.read_line(srcPath)
    prename = "Main.c"

    testDataPath = os.path.join(util.rootTestPath, versionId, "test_data", "inputs")
    ...
    # lineNum, sus_oc, sus_tu, sus_op, sus_ds, sus_jac, SUnion, SIntersection, SFrequency, covinfo, resinfo = SBFL.work(source,prename,testDataPath)
    # if lineNum == None:
    #     return
    # time_end = time.time()
    ...
    if True:
        ...
        if not os.path.exists(MBFLResultsPath):
            time_start = time.time()
            lineMax, sus_jac, sus_oc, sus_op, sus_tu, sus_ds, sus_MUSE = MBFL.work(source, prename, testDataPath)
            time_end = time.time()
...
if __name__ == "__main__":
    ...
    # with open('./Codeflaws_Filter_Data.pkl', 'rb') as file:
    ...
    for proId in proIdList:
        ...
        # versionDir = os.path.join(proIdDir, "test_data", "defect_root")
